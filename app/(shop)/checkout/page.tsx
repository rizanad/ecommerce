...
export default function CheckoutPage() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Basic Card Formatting (adds spaces every 4 digits)
    if (name === "cardNumber") {
  ...
  const validateStep1 = () => {
    return formData.firstName && formData.lastName && formData.email.includes("@") && formData.address;
  };
          ...
          <section className={`bg-white rounded-[2rem] p-8 border transition-all duration-500 ${step === 1 ? "border-violet-600 shadow-xl shadow-violet-100" : "border-slate-100 opacity-60"}`}>
            ...
            {step === 1 && (
              <div className="grid grid-cols-2 gap-4">
                <input name="firstName" required value={formData.firstName} onChange={handleInputChange} type="text" placeholder="First Name *" className="col-span-1 p-4 bg-slate-50 rounded-xl outline-none focus:ring-2 ring-violet-200 text-sm" />
                <input name="lastName" required value={formData.lastName} onChange={handleInputChange} type="text" placeholder="Last Name *" className="col-span-1 p-4 bg-slate-50 rounded-xl outline-none focus:ring-2 ring-violet-200 text-sm" />
                <input name="email" required value={formData.email} onChange={handleInputChange} type="email" placeholder="Email Address *" className="col-span-2 p-4 bg-slate-50 rounded-xl outline-none focus:ring-2 ring-violet-200 text-sm" />
                <input name="address" required value={formData.address} onChange={handleInputChange} type="text" placeholder="Full Delivery Address *" className="col-span-2 p-4 bg-slate-50 rounded-xl outline-none focus:ring-2 ring-violet-200 text-sm" />
                <button 
          ...
          <section className={`bg-white rounded-[2rem] p-8 border transition-all duration-500 ${step === 2 ? "border-violet-600 shadow-xl shadow-violet-100" : "border-slate-100 opacity-60"}`}>
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                >
                  {isProcessing ? "Processing..." : `Confirm & Pay Rs. ${subtotal}`}
                </button>
              </div>
            )}
          </section>
